...
export default function ServiceCard({ s }: { s: ServiceItem }) {
    const Icon = s.icon;
    return (
        <article className="group h-full rounded-xl border border-obsidianGray/60 bg-midnightBlack/50 p-4 md:p-4 shadow-[inset_0_1px_0_rgba(255,255,255,0.04)]">
            {/* empilha e centraliza até xl; lado a lado no xl+ */}
            <div className="flex h-full flex-col items-center justify-center gap-3 xl:flex-row xl:items-start xl:justify-start">
                {Icon && (
                    <span
                        aria-hidden
                ...
                {/* torna-se um col flex que cresce; botão vai ao rodapé com mt-auto */}
                <div className="flex-1 w-full flex flex-col gap-4">
                    <div className="flex flex-col gap-1.5">
                        <h3 className="text-base md:text-lg font-medium text-textApp text-center xl:text-left">
                            {s.title}
                        </h3>
                        <p className="text-sm text-textApp/70 text-center xl:text-left">
                            {s.desc}
                        </p>
                    </div>
                    ...
                    {s.href && (
                        <div className="mt-auto flex justify-center xl:justify-start">
                            <Button
                                href={s.href}
